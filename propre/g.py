...
# ─────────────────────────────────────────
# FIGURE 13 — DISTRIBUTIONS DES VARIABLES (FIX ROBUSTE)
# ─────────────────────────────────────────
# ─────────────────────────────────────────
# FIGURE 13 — DISTRIBUTIONS DES VARIABLES RETIRÉES
# ─────────────────────────────────────────

from nettoyage import load_mydf

df_raw = load_mydf()

# renommage manuel des colonnes à afficher
rename_fig13 = {
    "Number of Earths required":        "Nombre de Terres requises",
    "Number of Countries required":     "Nombre de pays requis",
    "Ecological (Deficit) or Reserve":  "Solde écologique",
    "BuiltUp_Footprint_Production":     "Empreinte urbain (prod.)",
    "BuiltUp_Footprint_Consumption":    "Empreinte urbain (conso.)",
    "BuiltUp_Biocapacity":              "Biocapacité urbain",
    "Cropland_Footprint_Production":    "Empreinte cultures (prod.)",
    "Cropland":                         "Biocapacité cultures",
}

cols_fig13 = [c for c in rename_fig13.keys() if c in df_raw.columns]

for col in cols_fig13:
    nom_fr = rename_fig13[col]
    data = pd.to_numeric(df_raw[col], errors="coerce").dropna()

    fig, ax = plt.subplots(figsize=(8, 5))
    ax.grid(False)

    sns.histplot(data=data, kde=True, bins=20, color=VERT_FONCE, ax=ax)

    ax.set_title(nom_fr, fontsize=14, color=VERT_FONCE)
    ax.set_xlabel(nom_fr, fontsize=9, color=GRIS_FONCE)
    ax.set_ylabel("Fréquence", fontsize=9, color=GRIS_FONCE)
    ax.spines["top"].set_visible(False)
    ax.spines["right"].set_visible(False)

    plt.tight_layout()
    safe_name = col.replace(" ", "_").replace("(", "").replace(")", "")
    plt.savefig(f"fig13_hist_{safe_name}.png", dpi=150, bbox_inches="tight")
    plt.close()

print(f"{len(cols_fig13)} histogrammes générés.")